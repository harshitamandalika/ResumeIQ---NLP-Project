 ...
 *   },
 *   skills: {
 *     validated_skills: { skill: string, evidence: string, evidence_lines?: string[] }[],
...
function renderResults(data) {
    ...
    const scoredBullets = quality.bullet_scores || [];
    const rewriteCandidates = quality.rewrite_candidates || filterActionableBullets(scoredBullets);
    ...
    document.getElementById('st-bullets').textContent = scoredBullets.length;
    ...
    renderBullets(rewriteCandidates);
    showPage('results');
}

function getSkillEvidenceText(skill) {
    if (Array.isArray(skill.evidence_lines) && skill.evidence_lines.length) {
        return skill.evidence_lines[0];
    }

    if (Array.isArray(skill.evidence) && skill.evidence.length) {
        return skill.evidence[0];
    }

    if (typeof skill.evidence === 'string' && skill.evidence.trim()) {
...
function getBulletText(bullet) {
    return bullet.bullet || bullet.text || '';
}

function getSuggestedRewrite(bullet) {
    return bullet.suggested_rewrite || bullet.rewrite || '';
}

function filterActionableBullets(bullets) {
    return bullets.filter((bullet) => getSuggestedRewrite(bullet) || bullet.metric_prompt || bullet.needs_user_metric);
}
...
function renderSkills(validated, missingSkills) {
    ...
    if (validated.length) {
        verifiedSkillsEl.innerHTML = validated.map((skill) => {
            const evidenceText = getSkillEvidenceText(skill);
            const percent = evidenceText ? 80 : 45;
            return `
                <div class="skill-item">
                    <div class="skill-left">
                        <div class="skill-name">${escHtml(skill.skill)}</div>
                        <div class="skill-evidence">${escHtml(evidenceText || 'Found in experience')}</div>
...
function renderBullets(bullets) {
    ...
    if (!bullets.length) {
        bulletsEl.innerHTML = '<p class="empty-note">No experience bullets currently need rewriting.</p>';
        return;
    }
    ...
    bulletsEl.innerHTML = bullets.map((bullet) => {
        ...
        const issuesHtml = bullet.issues && bullet.issues.length
            ? `<div>
                     ...
                     <div class="bc-issue-list">
                         ${bullet.issues.map((issue) => `<span class="bc-issue">${escHtml(formatIssue(issue))}</span>`).join('')}
                     </div>
                 </div>`
            : '';
        ...
        const metricPromptHtml = bullet.metric_prompt
            ? `<div class="bc-rewrite">
                     <div class="bc-rewrite-label">Suggested metric</div>
                     <div class="bc-rewrite-text">${escHtml(bullet.metric_prompt)}</div>
                 </div>`
            : '';
        ...
        return `
            <div class="bullet-card">
                ...
                <div class="bc-original">${escHtml(bulletText)}</div>
                ${issuesHtml}
                ${rewriteHtml}
                ${metricPromptHtml}
            </div>`;
